...
export class CustomerService {
  customersUrl = 'http://localhost:8181/api/customers';
  ...
  login(customer: Customer) {
    const loginUrl = 'http://localhost:8181/api/customers/login';
    return this.httpClient.post(loginUrl, customer);
  }

  getCustomer(id: any) {
    const searchCustomerUrl = 'http://localhost:8181/api/customers/' + id;
    return this.httpClient.get<Customer>(searchCustomerUrl);
  }

  getCustomerOrders(id: any) {
    const customerOrdersUrl =
      'http://localhost:8181/api/customers/' + id + '/orders';
  ...
  forgotPassword(email: string) {
    const url = 'http://localhost:8181/api/customers/forgot-password/' + email;
